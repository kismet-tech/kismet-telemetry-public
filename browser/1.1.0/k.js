/** Kismet browser telemetry 1.1.0. Fixed release; upgrades require a new URL. */
;
(function () {
    'use strict';
    ...
    function whenBody(fn) {
        if (document.body)
            return fn();
        var iv = setInterval(function () { if (document.body) {
            clearInterval(iv);
            fn();
        } }, 50);
    }
    ...
    function shouldSetCookies(cc) {
        return typeof cc === 'string' && /^[A-Z]{2}$/.test(cc) && cc !== 'XX' && cc !== 'ZZ' && !EU[cc];
    }
    ...
    function setConsentDefaults(cc) {
        var granted = shouldSetCookies(cc);
    ...
    function sendPageView() {
        ...
        try {
            if (navigator.sendBeacon) {
                navigator.sendBeacon(ORIGIN + TRACK_PATH, new Blob([JSON.stringify(payload)], { type: 'text/plain' }));
            }
            else {
                var xhr = new XMLHttpRequest();
    ...
    function startLeadFormStamping() {
        ...
        try {
            document.addEventListener('submit', function (e) { stampLeadForm(e.target); }, true);
        }
        catch (e) { }
            ...
            (function (evtName, map) {
                window.addEventListener('kismet:visitor:' + evtName, function (e) {
                    try {
                        ...
                        var capiConsent = _config && shouldSetCookies(_config.countryCode || '');
                        ...
                        if (window.fbq && map.metaEvent) {
                            ...
                            window.fbq('track', map.metaEvent, md, { eventID: eid });
                        }
                        if (map.ga4Event) {
                            var gi = {
                ...
                window.addEventListener('kismet:visitor:' + evtName, function (e) {
                    try {
                        ...
                        if (evtName === 'checkout:add-payment' && window.fbq) {
                            ...
                            if (eid) {
                                window.fbq('track', 'AddPaymentInfo', md, { eventID: eid });
                            }
                            else {
                                window.fbq('track', 'AddPaymentInfo', md);
                            }
                        }
                        if (evtName === 'checkout:purchase' && window.fbq && d.metaEventId) {
                            ...
                            window.fbq('track', 'Purchase', pmd, { eventID: d.metaEventId });
                        }
                    }
                    catch (err) { }
                });
    ...
    try {
        ...
        history.pushState = function () { var r = _origPush.apply(this, arguments); try {
            onSpaRouteChange();
        }
        catch (e) { } return r; };
        var _origReplace = history.replaceState;
        history.replaceState = function () { var r = _origReplace.apply(this, arguments); try {
            onSpaRouteChange();
        }
        catch (e) { } return r; };
