...
This branch fixes those paths and removes the browser bootstrap's warm-cookie shortcut. The bootstrap now checks current consent for returning visitors and suppresses identity when the anchor fails or times out. The PHP pure suite passes all 46 checks after the fix. The post-fix HTTP run is pending.
